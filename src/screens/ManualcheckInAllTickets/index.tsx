...
const ManualCheckInAllTickets: React.FC = () => {
        ...
        const fetchTicketDetails = async () => {
            ...
            if (response?.offline) {
                ...
                if (response?.data && Array.isArray(response.data) && response.data.length > 0) {
                    ...
                    const isScanned = response.data.some((ticket: any) => ticket.checkin_status === 'SCANNED');
                ...
                if (response?.data?.status === 'SCANNED') { // Adjust based on your actual response structure
                    ...
                    try {
                        ...
                        if (updatedResponse?.data && Array.isArray(updatedResponse.data) && updatedResponse.data.length > 0) {
                            ...
                            const isScanned = updatedResponse.data.some((ticket: any) => ticket.checkin_status === 'SCANNED');
                            setCheckInSuccess(isScanned);
    ...
    const handleTicketStatusChange = async (ticketUuid: string, newStatus: string, scannedByInfo: any = null) => {
        ...
        if (newStatus === 'SCANNED') {
            try {
                const updatedTicket = updatedTickets.find((t: any) => t.uuid === ticketUuid);
                if (updatedTicket) {
