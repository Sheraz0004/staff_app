...
interface EventItem {
  ...
  _originalUuid?: string;
  [key: string]: any;
}

interface SectionCountdown {
  days: number;
  hours: number;
  mins: number;
}
...
interface EventSectionProps {
  ...
  onSectionPress: (section: EventSectionData) => void;
}
...
const CountdownTimer: React.FC<CountdownTimerProps> = ({ days, hours, mins }) => (
...
const SmallEventCard: React.FC<SmallEventCardProps> = ({ event, isFirst, onPress }) => (
  <TouchableOpacity
...
const UpcomingEventItem: React.FC<UpcomingEventItemProps> = ({ event, onPress }) => (
...
const EventSection: React.FC<EventSectionProps> = ({ section, onEventPress, onSectionPress }) => {
...
const EventsScreen: React.FC<EventsScreenProps> = ({ eventInfo, onEventChange }) => {
  ...
  const [eventSections, setEventSections] = useState<EventSectionData[]>([]);
  ...
  const fetchEvents = async () => {
    try {
      ...
      const allEvents = eventIds.map((id: any) => ({ eventUuid: id }));

      const transformedEvents = allEvents
        .filter((event: any) => event.uuid || event.eventUuid)
        .map((event: any) => {
      ...
      const dateBuckets: { today: EventItem[]; thisWeek: EventItem[]; upcoming: EventItem[] } = { today: [], thisWeek: [], upcoming: [] };
      let dateParseSuccessCount = 0;

      transformedEvents.forEach((event: EventItem) => {
      ...
      const happeningToday: EventItem[] = [];
      const comingUpThisWeek: EventItem[] = [];
      ...
      if (useDateBased) {
        ...
        transformedEvents.forEach((event: EventItem, index: number) => {
  ...
  const handleSectionPress = (section: EventSectionData) => {
