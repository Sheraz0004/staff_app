...
import { Text, View, FlatList, TouchableOpacity, Alert, ListRenderItemInfo } from 'react-native';
...
interface CheckInAllPopupProps {
    ...
    onTicketStatusChange?: (uuid: string, status: string, scannedBy: ScannedBy | null) => void;
...
const CheckInAllPopup: React.FC<CheckInAllPopupProps> = ({
    ...
    const handleStatusChange = async (ticketToCheckIn: TicketItem) => {
        ...
        try {
            ...
            const response = await ticketService.manualDetailCheckin(
    ...
    const renderItem = ({ item }: ListRenderItemInfo<TicketItem>) => (
        <TouchableOpacity style={styles.ticketContainer} onPress={() => handleItemPress(item)}>
            <View>
